#ifndef SHINGO_PACKET_H
#define SHINGO_PACKET_H
...
public:
   ...
   */
  TypeHeader (MessageType t = SHINGO_IARP);
...
/**
 * \ingroup iarp
 * \brief IARP Update Packet Format
 * \verbatim
 |      0        |      1        |      2        |       3       |
...
class IarpHeader : public Header
{
 public:
  /**
   * Constructor
   ...
   */
  IarpHeader (Ipv4Address dst = Ipv4Address (), uint32_t hopcount = 0, uint32_t dstSeqNo = 0);
  virtual ~IarpHeader ();
 static  TypeId GetTypeId ();
 TypeId  GetInstanceTypeId () const;
 
   //カプセル化後のヘッダサイズを取得する関数
 uint32_t GetSerializedSize () const;
...
std::ostream & operator<< (std::ostream & os, const IarpHeader & packet);
...
public:
   ...
   */
   RreqHeader (uint8_t flags = 0, uint8_t reserved = 0, uint8_t hopCount = 0,
              ...
              uint32_t originSeqNo = 0/*, Ipv4Address rad = Ipv4Address()*/);
...
/*
  void SetRad (Ipv4Address a)
  {
    m_rad = a;
  }
  Ipv4Address GetRad () const
  {
    return m_rad;
  }
*/
